...
public class TpnaServiceImpl implements TpnaService {
	...
	public boolean poDetails(List<PolineModel> polineList) {
		...
		for (PolineModel poline : polineList) {
			...
			if (CommonValidation.isNullOrEmpty(poNumber) || CommonValidation.isNullOrEmpty(orderType)
			...
			if (currentOrd == null || currentOrd.getId() == 0L) {
				...
				if (orderType!=null && orderType.equalsIgnoreCase("C")) {
					currentOrd.setOrderPriority("1");
				} else if (orderType!=null && orderType.equalsIgnoreCase("E")) {
					currentOrd.setOrderPriority("2");
				} else if (orderType!=null && orderType.equalsIgnoreCase("D")) {
					currentOrd.setOrderPriority("3");
				} else if (orderType!=null && orderType.equalsIgnoreCase("V")) {
			...
			PartEntity part;
			if (orderExists) {
				part = partRepo.findByOrderIdAndPartNumberAndLineItemNumberAndDeliveryDueDate(currentOrd.getId(),
						partNumber, poLine, dddOrgDate);
			...
			if (!vendorCodeMap.containsKey(poline.getvDR_CD())) {
				...
				vendorEntity = vendorRepo.findByVendorCodeAndTradingPartnerId(poline.getvDR_CD(),
						poline.geteDI_PRTNR_ID());
